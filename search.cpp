...
#ifdef TUNING_MODE
int aspi_margin = 44, max_aspi_margin = 2000, aspi_constant = 49;
float aspi_mul = 195;									 // x100
int rfp_max_depth = 17, rfp_margin = 185, rfp_impr = 84; // rfp_max_depth unused (may retune later)
int iid_reduction_d = 2;
int dprune = 2069;														// DISABLED
int nmp_const = 4, nmp_depth = 11, nmp_evalmin = 44, nmp_evaldiv = 787; // evalmin DISABLED
float lmr_const = 6389, lmr_mul = 572;									// x10000; actually the lmr_mul is for log(d)*log(m)
...
uint32_t lmr_history_threshold = 3423;

// Extra LMR parameters, for better LMR (TODO: try removing, since they're always tiny)
float lmr_sqrt_mul = 1347, lmr_dist_mul = 69, lmr_depth_mul = 253; // all x10000
...
int16_t see_noisy = 30, see_quiet = 87; // in centipawns; noisy: *depth^2 (quad); quiet: *depth (linear)

// LMP parameters (all x10000)
double lmp_noimpr_const = 25997, lmp_noimpr_linear = 4295, lmp_noimpr_quad = 8683;
double lmp_impr_const = 42956, lmp_impr_linear = 11419, lmp_impr_quad = 11856;

// QS stuff
int qs_chk = 0; // check ply
int16_t qs_see_nochk = 83;
int16_t qs_see_chk = 30;
#endif
...
void init_search() // Initialize the late move reduction table
{
	for (uint8_t depth = 0; depth < MAX_DEPTH; depth++)
		stack.nnue_data->accumulator.computedAccumulation = 0;

	for (uint8_t depth = 0; depth < MAX_DEPTH; depth++) // prev ply
		for (uint8_t move = 0; move < MAX_MOVE; move++)
		{
			if (depth < 1)
				lmr_table[depth][move] = 0; // no reduction at too little depth (prev LMR_MINDEPTH)
			else if (move < 1)
				lmr_table[depth][move] = 0; // no reduction for first few moves (prev LMR_THRESHOLD)
			else
			{
...
bool nullmove_safe(uint8_t stm)
{
	...
	for (uint8_t plist_idx = (stm & 16) ^ 17; plist_idx & 15; plist_idx++) // iterate through all friendly pieces (except for the king)
	{
		...
		if (nullmove_counter <= 0)
			return true; // Null move is safe: zugzwang unlikely
	}
...
int16_t search(uint8_t stm, uint8_t depth, uint8_t last_target, int16_t alpha, int16_t beta, uint64_t hash, int8_t nullmove, uint8_t ply, int8_t last_zeroing_ply)
{
	...
	if (ply - last_zeroing_ply >= 100)
		return FIFTY_MOVE; // if we've gone over 100 ply without zeroing, then the game is a draw
	...
	if (last_target == (uint8_t)-2)
		key ^= Z_DPP(last_target);
	...
	if (incheck && ply)
		static_eval = -eval_stack[ply - 1]; // avoid using static eval directly when in check
	...
	if (incheck && depth > CHKEXT_MINDEPTH)
	...
	if (mlist.count == 0)
		return incheck ? MATE_SCORE + ply : STALEMATE; // checkmate or stalemate
	...
	while (mlist.count) // iterate through it backwards
	{
		...
		if (curmove.score < LOUD_MOVE && depth < LMP_MAXDEPTH && legal_move_count >= lmp_table[depth][improving] && !is_pv && !incheck && ply
			&& static_eval - alpha <= depth * 100 + 100) //avoid LMP cutting away the only good move at low depth
		{
		...
		if ((res.prev_state & PTYPE) < 3 || (curmove.flags & F_CAPT))
			updated_last_zeroing_ply = ply; // if we moved a pawn, or captured, the HMC is reset
		...
		if (!incheck && curmove.score < LOUD_MOVE)
			lmr_move_count++; // variable doesn't get increased if in check or if there are tactical moves
		else
			lmr = 0; // don't do LMR in check or on tactical moves
		...
		if (eval > best_score) // new best move!
		{
			...
			best_move = curmove;
			if (!ply)
				best_node_count = node_count - last_node_count; // best move's subtree: save node count
			...
			if (eval > alpha) // beat alpha
			{
				...
				if (alpha >= beta) // beta cutoff
				{
					...
					if (panic)
						return 0; // should NOT set TT entries when out of time!
...
int16_t qsearch(uint8_t stm, int16_t alpha, int16_t beta, int8_t check_ply, uint64_t hash)
{
	...
	else
	{
		...
		if (mlist.count == 0)
			return alpha; // No captures available: return alpha
	}
	...
	if (no_legal_move)
		return MATE_SCORE + MAX_DEPTH; // checkmate: in check and no legal move
...
void search_root(uint32_t time_ms, bool movetime, bool infinite, uint8_t fixed_depth)
{
	if (!infinite)
		...
		search_end_time = (long)((1U << 31) - 1);		// infinite time
	clock_t flag_time = clock() + total_remaining_time; // use in time manager to avoid flagging
	stack.ply = 0;
	...
	for (uint8_t depth = 1; depth < std::min(fixed_depth, (uint8_t)MAX_DEPTH); depth++) // increase depth until MAX_DEPTH reached
	{
		...
		do
		{
			while (eval <= alpha || eval >= beta)
			{
				...
				{
					if (eval <= alpha)
						alpha_margin = alpha_margin * ASPI_MULTIPLIER + ASPI_CONSTANT;
					else if (eval >= beta)
						beta_margin = beta_margin * ASPI_MULTIPLIER + ASPI_CONSTANT;
		...
		if (panic)
			break; // we do not have any more time!
		...
		if (!movetime && !infinite) // time management in search: disable with fixed time search
		{
			...
			if (eval >= MAX_DEPTH - MATE_SCORE)
				break; //!= because in qsearch, ply is MAX_DEPTH
			...
			if (search_end_time - end <=
					TM_CUTOFF_MUL * (end - start) + TM_CUTOFF_MUL2 * prev_search_time + (TM_CUTOFF_CONST * CLOCKS_PER_SEC / 1000) &&
				!benchmark)
				break;
			...
			float frac = ((float)best_node_count / node_count);
			frac -= TM_NODEFRAC_CONST; // remove some of it
			frac *= TM_NODEFRAC_MUL;   // attenuate it a bit
			// frac = std::max(frac, 0.f); //clip it to avoid it being negative and adding time
