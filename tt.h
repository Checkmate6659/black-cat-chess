...
#else
        if(100 * depth + 200 * is_pv <= 100 * entry.depth - 20 * age - 300) //weird that using std::min(entry.depth, 3) doesn't give the same result, but changing <= to < doesn't change bench
#endif
            return;
    }
